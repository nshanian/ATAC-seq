## ATAC-seq raw data analysis 

## Alignment and Peak Calling

This repository contains workflows and tools for performing alignment and peak calling on paired-end (PE) ATAC-seq data in `.fastq` file format generated by next-generation sequencing (NGS).
...
The workflows are written to run as containerized `SLURM` submission scripts on a high performance computing (HPC) cluster using `sbatch` `script.sh`. 

**NOTE:** `ATAC_align.sh` (step 1) and `ATAC_peakcall.sh` (step 2) workflows have to be run sequentially, as step 1 output files are necessary as input for step 2.

The first `ATAC_align.sh` workflow uses `bowtie2` (v2.3.1) and the second `ATAC_peakcall.sh` uses `MACS2` (v2.1.0), based on the GRCh38/hg38 assembly of the human genome:
...
https://hgdownload.cse.ucsc.edu/goldenPath/hg38/chromosomes/
  ...
  For documentation on a ATAC-seq pipeline from the ENCODE consortium see:
